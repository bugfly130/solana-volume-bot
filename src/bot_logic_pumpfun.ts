...
import { calcAmountOut, getBuyInstructions, getSellInstructions } from './pumpfun/pumpfun_swap';
...
export const botRunBundle = async (chatid: string, addr: string) => {
  console.log('*************pumpfun botRunBundle*************');
  ...
  for (let i = 0; i < 2; i++) {
    ...
    const tokenAmount = Math.floor(await calcAmountOut(depositer, addr, buyAmount, true) * 80 / 100)
    const buyInst = await buy(depositer, addr, tokenAmount * (10 ** 6), buyAmount * LAMPORTS_PER_SOL);
    ...
    if (buyInst && buyInst.instructions) {
      ...
      const sellTokenAmount = buyInst.amountToken + Number(restTokenBalance);
      const solAmount = Math.floor(await calcAmountOut(depositer, token.addr, sellTokenAmount / (10 ** 6), false) * LAMPORTS_PER_SOL * 50 / 100);
      const sellInst = await sell(depositer, addr, sellTokenAmount, solAmount);
...
export const stopBot = async (chatid: string, addr: string) => {
  ...
  if (tokenBalance) {
    const solAmount = Math.floor(await calcAmountOut(depositer, token.addr, Number(tokenBalance) / (10 ** 6), false) * LAMPORTS_PER_SOL * 50 / 100);
    const sellInst = await sell(depositer, addr, Number(tokenBalance), solAmount);
...
export async function buy(payerWallet: Keypair, tokenAddress: string, amount: number, maxSolCost: number) {
  try {
    const instructions = await getBuyInstructions(payerWallet, tokenAddress, amount, maxSolCost);

    return { instructions: instructions, amountToken: amount }
  } catch (err) {
    G.log('❌ Buy error ->', err);
  }
}

export async function sell(payerWallet: Keypair, tokenAddress: string, amount: number, minSolOutput: number) {
  try {
    const instructions = await getSellInstructions(payerWallet, tokenAddress, amount, minSolOutput);

    return {
      instructions,
      amountToken: minSolOutput,
    };
  } catch (err) {
