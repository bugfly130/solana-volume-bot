...
import { PoolFetchType, CurveCalculator, Raydium, PoolUtils, SOLMint } from "@raydium-io/raydium-sdk-v2";
import { raydiumSDKList } from './amm/constants';
import { isValidClmm } from './utils/sdkv2';
...
export async function getPoolInfo(raydium: Raydium | undefined, token: string) {
  ...
  try {
    ...
    for (let i = 0; i < poolNum; i++) {
      if (isValidClmm(data.data[i].programId)) {
        return data.data[i];
      }
    }
  } catch {
...
export const botRunBundle = async (chatid: string, addr: string, pool: any) => {
  ...
  for (let i = 0; i < 2; i++) {
    ...
    const buyInst = await buyCLMM(raydiumSDKList.get(depositer.publicKey.toBase58()), token.addr, pool, depositer, buyAmount);
    ...
    if (buyInst) {
      ...
      const sellInst = await sellCLMM(raydiumSDKList.get(depositer.publicKey.toBase58()), token.addr, pool, depositer, sellTokenAmount);
...
export const stopBot = async (chatid: string, addr: string, pool: any) => {
  ...
  if (tokenBalance) {
    const sellInst = await sellCLMM(raydiumSDKList.get(depositer.publicKey.toBase58()), token.addr, pool, depositer, Number(tokenBalance));
...
export async function buyCLMM(raydium: Raydium | undefined, token: string, poolInfo: any, payerWallet: Keypair, amount: number) {
  ...
  try {
    ...
    return { instructions: transaction.instructions, amountToken: minAmountOut.amount.raw.toNumber() };
  } catch (err) {
    G.log('❌ Buy error ->', err);
  }
}

export async function sellCLMM(raydium: Raydium | undefined, token: string, poolInfo: any, payerWallet: Keypair, amount: number) {
  if (raydium == undefined) return null;
  try {
    const clmmPoolInfo = await PoolUtils.fetchComputeClmmInfo({
      connection: raydium.connection,
