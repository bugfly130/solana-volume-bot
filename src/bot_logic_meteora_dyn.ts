...
export const getPoolInfo = async (chatid: string, poolAddr: string) => {
  ...
  const pool = await AmmImpl.create(provider.connection, new PublicKey(poolAddr));
...
export const botRunBundle = async (chatid: string, addr: string, pool: any) => {
  ...
  for (let i = 0; i < 2; i++) {
    ...
    const buyInst = await buyDYN(pool, depositer, new BN(buyAmount));
    ...
    if (buyInst) {
      ...
      const sellInst = await sellDYN(pool, depositer, buyInst.amountToken.add(new BN(Number(restTokenBalance))));
      ...
      if (sellInst) {
        const sellTransaction = new VersionedTransaction(
          new TransactionMessage({
            payerKey: feeKeypairs[i].publicKey,
            recentBlockhash: '1',
            instructions: sellInst.instructions.slice(1),
          }).compileToV0Message(),
        );
...
export const stopBot = async (chatid: string, addr: string, pool: any) => {
  ...
  if (tokenBalance) {
    const sellInst = await sellDYN(pool, depositer, new BN(Number(tokenBalance)));
...
export async function buyDYN(pool: AmmImpl, payerWallet: Keypair, solAmount: BN) {
  try {
    ...
    let swapQuote = pool.getSwapQuote(inTokenMint, solAmount, 5);
...
export async function sellDYN(pool: AmmImpl, payerWallet: Keypair, tokenAmount: BN) {
  try {
    const swapAtoB = true;
    let swapInToken = swapAtoB ? pool.tokenAMint : pool.tokenBMint;
    let inTokenMint = new PublicKey(swapInToken.address);
    let swapQuote = pool.getSwapQuote(inTokenMint, tokenAmount, 5);
